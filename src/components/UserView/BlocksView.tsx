...
function BlocksView (props: Props): ReactElement {
...
export default withRouter(BlocksView);
...
async function queryNext(username: string, next: number | null): Promise<Pageable<DomainEnvelope<DomainConnection>, number>> {
  ...
  const resp = await fetch(`${INDEXER_API}/users/${username}/blockees?limit=20${next ? '&offset=' + next : ''}`);
