...
function FollowingView (props: Props): ReactElement {
...
export default withRouter(FollowingView);
...
async function queryNext(username: string, next: number | null): Promise<Pageable<DomainEnvelope<DomainConnection>, number>> {
  ...
  const resp = await fetch(`${INDEXER_API}/users/${username}/followees?limit=20${next ? '&offset=' + next : ''}`);
