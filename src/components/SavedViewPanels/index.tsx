...
import {CustomViewProps} from "../../utils/types";
...

      {
        !!savedViews?.length && (
          <div className="search-panel saved-views-panels">
            ...
            {savedViews.map((savedView: CustomViewProps, i: number) => (
              renderSaveViewRow(savedView, i, props))
            )}
          </div>
        )
      }
          ...
          editing
            ? (
              <Input
                ...
                onChange={(e: MouseEvent) => setNewTitle((e.target as HTMLInputElement).value)}
...
function useUpdateViewProps(viewIndex: number): CustomViewProps | null {
  ...
  filterUsers?.forEach((u: string) => newUserSet.add(u));
  filterTags?.forEach((t: string) => newTagSet.add(t));
  ...
  const isUsersEqual = filterUsers.length === newUserSet.size
    && filterUsers.reduce((equal: boolean, user: string) => (
      equal && newUserSet.has(user)
    ), true);

  const isTagsEqual = filterTags.length === newTagSet.size
    && filterTags.reduce((equal: boolean, tag: string) => (
